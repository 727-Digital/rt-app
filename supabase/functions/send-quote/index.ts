...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const smsBody = `Hi ${lead.name}! Your turf installation quote from Reliable Turf is ready. View it here: ${quoteUrl}`;
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;margin:0;padding:20px;background:#f5f5f5;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;padding:32px;box-shadow:0 1px 3px rgba(0,0,0,.1);">
    ...
    <table style="width:100%;border-collapse:collapse;margin:16px 0;">
      ...
      <tfoot>
        <tr>
          ...
          <td style="padding:12px;text-align:right;font-weight:700;font-size:18px;color:#16a34a;">${formatCurrency(quote.total)}</td>
        </tr>
      </tfoot>
    </table>
    ...
    const emailSubject = `Your Turf Quote from Reliable Turf - ${formatCurrency(quote.total)}`;
